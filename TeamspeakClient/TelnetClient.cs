...
namespace TentacleSoftware.Telnet {
    public class TelnetClient : IDisposable {
        ...
        /// </summary>
        /// <returns></returns>
        public async Task Connect(Encoding inbound = null, Encoding outbound = null) {
            if (_tcpClient != null) {
                throw new NotSupportedException($"{nameof(Connect)} aborted: Reconnecting is not supported. You must dispose of this instance and instantiate a new TelnetClient");
            }

            if (inbound == null) inbound = Encoding.UTF8;
            if (outbound == null) outbound = Encoding.UTF8;

            _tcpClient = new TcpClient();
            await _tcpClient.ConnectAsync(_host, _port);

            _tcpReader = new StreamReader(_tcpClient.GetStream(), inbound);
            _tcpWriter = new StreamWriter(_tcpClient.GetStream(), outbound) { AutoFlush = true };
        ...
        public async Task Send(string message) {
            try {
                ...
                // Send command + params
                await _tcpWriter.WriteAsync(message);
